...
def get_model():
    model = Sequential()
    model.add(Conv3D(64, (3, 3, 3), padding='same', activation='relu', input_shape=(40, 224, 224, 1)))
...
parallel_model.fit_generator(data.data_generator(X_train, 'standard', size=size, batch_size=batch_size),
                                 ...
                                 validation_steps=valid_steps , workers=38)#, initial_epoch=0 ,use_multiprocessing= True)
